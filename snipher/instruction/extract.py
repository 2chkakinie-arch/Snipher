...
_LABEL_KEY = re.compile(r"([一-龯ァ-ヶーぁ-んA-Za-z][一-龯ァ-ヶーぁ-んA-Za-z0-9_ ()（）]{0,14})"
                        r"\s*[:：]")
#: ラベルの値が *指示文* まで飲み込まないための切り口（v8: Q8 の事故の再発防止）。
#: 「名前：みのお」という情報を、キーを…」→ 値は みのお で止める。
_VALUE_CUT = re.compile(r"[」』\"']|という|との|に関する|についての|"
                        r"(?:出力|変換|抽出|要約|作成|作って|列挙|まとめ|点検|確認|判定|検証|書いて|直して|並べて)"
                        r".{0,8}(?:してください|して下さい|しなさい|せよ|しろ|ください|下さい|して)|"
                        r"(?:してください|して下さい|しなさい|せよ|しろ|ください|下さい|お願いします)")
...
def label_values(text: str) -> dict[str, str]:
    ...
    for i, m in enumerate(hits):
        ...
        # 直後の文（別の材料）まで飲み込まない
        val = re.split(r"[。\n]", val)[0].strip(" 　、。,，")
        # 引用の閉じ・メタ言語（という）・指示の動詞で止める（指示文の飲み込み防止）
        cut = _VALUE_CUT.split(val, maxsplit=1)[0].strip(" 　、。,，")
        if cut:
            val = cut
