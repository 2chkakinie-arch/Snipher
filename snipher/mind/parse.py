...
ASK_KINDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("definition", ("とは", "って何", "とは何", "どういうもの", "どんなもの", "何者",
                    "定義", "意味は何", "どういう意味", "どんな意味", "何のこと", "いったい何")),
    ...
    ("count", ("いくつ", "何個", "何匹", "何本", "何人", "何枚", "何文字", "何歳", "何点",
               "何億", "何万", "何百万", "何ギガ", "何テラ", "何バイト", "何gb", "何パラメータ")),
...
def detect_act(text: str, *, ask: str = "") -> str:
    ...
    if tail.endswith(("?", "？")) or _Q_END.search(tail) or ask:
        return "ask"
    # 「〜はどんな旗」のような疑問詞を中身に入れた問い（句点/語尾に疑問形が無い場合も）。
    # 「どんなに」は感嘆（どんなに待っても）なので除外する。
    if re.search(r"(どんな(?!に)|どれ(?!くらい)|どっち|どの|何[種類個円万億匹本枚人歳点文字音])", t):
        return "ask"
...
# --------------------------------------------------------------------------- #
# 「使い方」や「例文」は *語の素材* を聞く質問ではなく、対象の procedures を聞く質問です。
# ここに "example" を入れると、辞書の 表記・読み・拍・品詞 が答えとして出てしまいます。
_WORD_INFO_ASK = ("reading", "word_property", "synonym", "antonym", "word_list")
